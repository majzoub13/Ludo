...
class Algorithims:
    ...
    def expectimax(self, state, depth, max_player, min_player, number):
        ...
        all_states = self.all_possible_states(
            state, max_player if is_maximizing else min_player, number
        )
        ...
        for new_state in all_states:
            ...
            for dice_roll in range(1, 7):
                future_state = self.expectimax(
                    new_state, depth - 1, min_player, max_player, dice_roll
                )
    ...
    def all_possible_states(self, state, number):
        ...
        for piece in player.get_movable_pieces():
            new_state = copy.deepcopy(state)
            self.logic.check_and_move(new_state.board, player, piece, number)
            states.append(new_state)
        return states
